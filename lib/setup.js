...
const { GIT_SAFETY_HOOK_TEMPLATE, generateSafeGitCommands } = require('./generators/git-safety');
const { SECURITY_CHECK_HOOK_TEMPLATE } = require('./generators/security-safety');
...
async function createFullSetup(projectPath, analysis, options = {}) {
  ...
  // Install git safety system
  await installGitSafety(projectPath);

  // Install security safety system
  await installSecuritySafety(projectPath);
...
async function installGitSafety(projectPath) {
  ...
  const hookPath = path.join(hooksDir, 'git-safety-check.sh');
  await fs.writeFile(hookPath, GIT_SAFETY_HOOK_TEMPLATE, 'utf8');
  ...
  console.log(chalk.green('  ✅ Git safety system installed'));
  console.log(chalk.gray('     • Hook: ~/.claude/hooks/git-safety-check.sh'));
  console.log(chalk.gray('     • Commands: /safe-switch, /safe-commit, /safe-reset, /git-safety-status'));
}

async function installSecuritySafety(projectPath) {
  const chalk = await getChalk();
  const homeDir = os.homedir();
  const hooksDir = path.join(homeDir, '.claude', 'hooks');

  // Create global hooks directory (already created by git safety, but ensure it exists)
  await fs.ensureDir(hooksDir);

  // Install security check hook
  const hookPath = path.join(hooksDir, 'security-check.sh');
  await fs.writeFile(hookPath, SECURITY_CHECK_HOOK_TEMPLATE, 'utf8');
  ...
  console.log(chalk.green('  ✅ Security safety system installed'));
  console.log(chalk.gray('     • Hook: ~/.claude/hooks/security-check.sh'));
  console.log(chalk.gray('     • Scans: SQL injection, eval(), hardcoded credentials, XSS'));
}
