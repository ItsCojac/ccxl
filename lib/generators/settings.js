...
const { generateSecuritySafety } = require('./security-safety');
...
function generateSettings(analysis, options = {}) {
  // Generate safety configurations
  const gitSafety = generateGitSafety(analysis, options);
  const securitySafety = generateSecuritySafety(analysis, options);
  ...
  const allowedTools = [
    ...
    // System info (git permissions handled by git-safety module, security hooks by security-safety module)
    "Bash(node --version:*)",
    "Bash(npm --version:*)",
    "Bash(uname:*)",
    "Bash(whoami:*)",
  ...
  // Merge all safety hooks with project hooks
  const mergedHooks = { ...gitSafety.hooks };

  // Merge security hooks (PostToolUse)
  if (securitySafety.hooks.PostToolUse) {
    if (mergedHooks.PostToolUse) {
      mergedHooks.PostToolUse = [...mergedHooks.PostToolUse, ...securitySafety.hooks.PostToolUse];
    } else {
      mergedHooks.PostToolUse = securitySafety.hooks.PostToolUse;
    }
  }
  ...
  const settings = {
    ...
    permissions: {
      allow: [
        ...
        ...(securitySafety.permissions.allow || [])
      ],
      ask: [...askPermissions, ...gitSafety.permissions.ask],
      deny: [
        ...deniedTools,
        ...gitSafety.permissions.deny,
        ...(securitySafety.permissions.deny || [])
      ],
      defaultMode: "default"
    }
  };
