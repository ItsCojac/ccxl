...
✅ **Safe**: Use JSON.parse for data, proper parsers for code

❌ **Hardcoded secrets**: \`const API_KEY = "sk-1234567890"\`
✅ **Safe**: \`const API_KEY = process.env.API_KEY\`

❌ **Command injection**: \`exec("git commit -m " + userMessage)\`
✅ **Safe**: Use libraries with proper escaping or parameterized commands
`;
}
...
for file in $FILES; do
    ...
    if grep -nE "(SELECT|INSERT|UPDATE|DELETE|FROM|WHERE).*\\+.*\\$|\\+.*\\(|exec\\(.*SELECT" "$file" 2>/dev/null; then
        echo -e "\${YELLOW}⚠️  Possible SQL injection in $file\${NC}"
    ...
    if grep -nE "\\beval\\s*\\(|\\bexec\\s*\\(" "$file" 2>/dev/null; then
        echo -e "\${YELLOW}⚠️  eval()/exec() detected in $file\${NC}"
    ...
    if grep -niE "(password|api[_-]?key|secret|token|auth)\\s*=\\s*['\\\"].{8,}" "$file" 2>/dev/null; then
        echo -e "\${RED}🚫 Possible hardcoded credentials in $file\${NC}"
    ...
    if grep -nE "exec\\(.*\\+|system\\(.*\\+|spawn\\(.*\\+" "$file" 2>/dev/null; then
        echo -e "\${YELLOW}⚠️  Possible command injection in $file\${NC}"
    ...
    if grep -nE "innerHTML\\s*=|dangerouslySetInnerHTML" "$file" 2>/dev/null; then
        echo -e "\${YELLOW}⚠️  XSS risk detected in $file\${NC}"
...
if [ "$WARNINGS" -gt 0 ]; then
    echo ""
    echo -e "\${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\${NC}"
    echo -e "\${YELLOW}⚠️  $WARNINGS security warning(s) found\${NC}"
    echo ""
