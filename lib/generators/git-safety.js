...
if echo "$COMMAND" | grep -qE "git (reset --hard|clean -fd|checkout [^-]|branch -D|push --force)"; then
    ...
    if ! git diff-index --quiet HEAD -- 2>/dev/null; then
        ...
        echo "  • git stash push -m 'before operation'"
        echo "  • /safe-switch <branch>  (for branch switching)"
        echo "  • /safe-reset <commit>   (for reset operations)"
        echo "  • /git-safety-status     (check full status)"
        echo ""
        exit 1
    fi
    ...
    if [ "$UNTRACKED" -gt 0 ] && echo "$COMMAND" | grep -q "clean -fd"; then
        echo -e "\${RED}🚫 BLOCKED: $UNTRACKED untracked files would be deleted\${NC}"
        echo ""
    ...
    if git branch "$SAFETY_BRANCH" 2>/dev/null; then
        echo -e "\${GREEN}✅ Safety backup created: $SAFETY_BRANCH\${NC}"
